...
/**
 * Customer service unit tests — CRUD rules without a database.
 *
 * Pattern: mock-require swaps repositories before the service loads.
 * Each test sets mock return values, calls the service, checks the result.
 */
...
mock("./customer.repository", customerRepository);
...
describe("getCustomerById", () => {
  ...
  it("throws 404 when the customer does not exist", async () => {
    ...
    await expect(customerService.getCustomerById("missing")).rejects.toMatchObject({
      statusCode: 404,
